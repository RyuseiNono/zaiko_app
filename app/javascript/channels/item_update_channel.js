...
consumer.subscriptions.create("ItemUpdateChannel", {
  ...
  received(data) {
    ...
    if (clone_new != null) {
      ...
      var item_price_new = document.getElementById(
        `item_price_new-#${item.id}`
      );
      ...
      item_price_new.value = `${item.price}`;
      ...
      var item_price_index = document.getElementById(
        `item_price_index-#${item.id}`
      );
      var item_count_index = document.getElementById(
        `item_count_index-#${item.id}`
      );
      item_name_index.textContent = item.name;
      item_price_index.textContent = `${item.price}円`;
      item_count_index.textContent = item.count;

      // countによる処理
      var new_item_name = update_item.getElementsByTagName("td")[0];
      var new_item_price = update_item.getElementsByTagName("td")[1];
      var new_item_count = update_item.getElementsByTagName("td")[2];
      if (item.count == 0) {
        ...
        new_item_price.classList.remove("text-muted");
