...
RSpec.describe 'Itemを登録する', type: :system do
  ...
  context '在庫を新規登録ができるとき', js: true do
    it '正しい情報を入力すれば在庫を新規登録がでる' do
      ...
      expect(page).to have_field 'item[name]', with: @item.name.to_s
      expect(page).to have_field 'item[price]', with: @item.price.to_s
      expect(page).to have_field 'item[count]', with: @item.count.to_s
...
RSpec.describe 'Itemを編集する', type: :system do
  ...
  context '在庫情報を編集できるとき' do
    it '正しい情報を入力すれば在庫を編集ができる' do
      ...
      sleep(0.1)
      # 編集した内容が表示されているか確認する
      expect(page).to have_field 'item[name]', with: @item_edit.name.to_s
      expect(page).to have_field 'item[price]', with: @item_edit.price.to_s
      expect(page).to have_field 'item[count]', with: @item_edit.count.to_s
